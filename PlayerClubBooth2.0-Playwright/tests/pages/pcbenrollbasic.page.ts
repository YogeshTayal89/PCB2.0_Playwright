...
export default class PCBEnrollBasicPage {
    ...
    currentdate:Locator;generation:Locator;generationvalue:Locator;dobpick:Locator;
    ...
    constructor(page: Page) {
        ...
        this.doberrormsg=this.framelocator.getByText('Please enter "DOB"');
      ...
      //  this.suffix = this.framelocator.getByText('Select Suffix');
        this.suffixvalue = this.framelocator.getByText('M.D.')
        this.generation = this.framelocator.locator('#mat-select-value-23').getByText('Select Generation')
        this.generationvalue = this.framelocator.getByText('JR')
        this.preferredname = this.framelocator.getByPlaceholder('Enter Preferred Name');
        this.gender = this.framelocator.getByText('Select Gender');
        this.gendermalevalue=this.framelocator.getByText('Male', { exact: true });
       // this.dobenter=this.framelocator.getByRole('textbox', { name: 'MM/DD/YYYY' });
        this.dobpick=this.framelocator.getByRole('textbox', { name: 'MM/DD/YYYY' });
        this.dob=this.framelocator.getByRole('button', { name: 'Open calendar' });
        ...
        this.taxid=this.framelocator.getByPlaceholder('Enter tax ID')
        this.crossref=this.framelocator.getByPlaceholder('Enter Cross Ref');
        ...
        this.nextbtn= this.framelocator.locator('#basicNext')
        //this.framelocator.getByRole('button', { name: 'NEXT'});
        ...
        //Locator for loading symbol
        this.loader=this.framelocator.getByRole('img', { name: 'Angular Logo' }).first();
    }
    ...
    async fillBasicEnrollmentDetails(testdata:any){
        ...
        await this.util.clickElement(this.suffixvalue);

         //Generation
         await this.util.clickElement(this.generation);
         await this.util.clickElement(this.generationvalue);
 
         await this.util.typeText(this.preferredname, testdata.preferredname);

        //Gender
        await this.util.clickElement(this.gender);
        await this.util.clickElement(this.gendermalevalue);
    ...
    async verifyEnterdBasicDetails(testdata:any){
       ...
       // await this.util.verifyTextPresent(this.gendermalevalue,testdata.gender);
