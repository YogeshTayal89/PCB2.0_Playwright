...
export default class PCBPlayerCardDetailsPage {
    ...
    pincancelbtn: Locator; pinsavebtn: Locator; pinupdatebtn: Locator; reenterpinchangelbl: Locator;
    ...
    //Reset Abandoned card
    abandonedstatus: Locator; abandonedheader: Locator; abandonedmsg: Locator; abandonednobtn: Locator; abandonedyesbtn: Locator; cardststatus: Locator;

    //Reset Pin lock
    pinlockbtn: Locator; pinlockstatus: Locator; pinlockheader: Locator; pinlockmsg: Locator; pinlockcancelbtn: Locator; pinlockokbtn: Locator;
    ...
    constructor(page: Page) {
        ...
        this.reenterpinlbl = this.framelocator.getByLabel('Set Card PIN').getByText('Re-enter PIN'); //
        this.reenterpinchangelbl = this.framelocator.getByLabel('Change Card PIN').getByText('Re-enter PIN'); //
        this.reenterpininput = this.framelocator.getByPlaceholder('Re-enter PIN', { exact: true }).first();
        ...
        this.enteroldpinlbl = this.framelocator.getByLabel('Change Card PIN').getByText('Enter Old PIN');
        ...
        //Reset Abandoned Status
        this.abandonedstatus = this.framelocator.getByText('Abandoned').first();
        this.abandonedheader = this.framelocator.getByRole('heading', { name: 'Reset Card Status' });
        this.abandonedmsg = this.framelocator.getByText('Are you sure you want to reset the Abandoned Card Status?')
        this.abandonednobtn = this.framelocator.getByRole('button', { name: 'No' });
        this.abandonedyesbtn = this.framelocator.getByRole('button', { name: 'Yes' });

        this.cardststatus = this.framelocator.getByLabel('Cards').getByText('Active');


        // Reset Pin Lock status
        this.pinlockbtn = this.framelocator.getByRole('button', { name: 'lock_open' });
        this.pinlockstatus = this.framelocator.getByText('PIN Lock').first();
        this.pinlockheader = this.framelocator.getByRole('heading', { name: 'Reset PIN Lock' })
        this.pinlockmsg = this.framelocator.getByText('Are you sure you want to reset the PIN Locked Status?')
        this.pinlockcancelbtn = this.framelocator.getByRole('button', { name: 'CANCEL' });
        this.pinlockokbtn = this.framelocator.getByRole('button', { name: 'OK' })
    ...
    /**
      * This method is used to verify the PIN LOCK status visible when pin is locked
     */
    async verifyPinLockStatus() {
        await this.util.verifyElementPresent(this.pinlockstatus);
    }


    /**
     * This method is used to verify When PIN is locked the “Edit" icon should be disabled to avoid PIN operations
     */
    async verifyActionButtonsAtPinLock() {
        await this.util.verifyElementPresent(this.pinlockbtn);
        await this.util.verifyDisabled(this.editbtn);
    }

    /**
     * This method is used to verify When PIN LOCK Pop up
     */
    async verifyPinLockPopUp() {
        await this.util.clickElement(this.pinlockbtn);
        await this.util.verifyElementPresent(this.pinlockheader);
        await this.util.verifyElementPresent(this.pinlockmsg);
    }

    /**
     * This method is used to click on Cancel button
     */
    async resetPinLockClickCancel() {
        await this.util.clickElement(this.pinlockcancelbtn);

    }
    /**
     * This method is used to click on OK button
     */
    async resetPinLockClickOk() {
        await this.util.clickElement(this.pinlockokbtn);
    ...
    /**
     * This method is used to validate the status after reseting pin lock
     */
    async verifyCardStatusAfterPinLockReset() {
        await this.util.verifyElementPresent(this.cardststatus);
        await this.util.verifyElementNotPresent(this.pinlockbtn);
        await this.util.verifyElementPresent(this.editbtn);

    }

    /**
     * This method is used to verify the Abandoned status visible when card is abondaned
    */

    async verifyAbandonedLockStatus() {
        await this.util.verifyElementPresent(this.abandonedstatus);
    }


    /**
    * This method is used to verify When card is Abandoned, only edit button should appear to reset status
    */
    async verifyActionButtonsAbandoned() {
        await this.util.verifyElementPresent(this.editbtn);

    }
    /**
     * This method is used to verify When Abandoned Pop up
     */
    async verifyAbandonedPopUp() {
        await this.util.clickElement(this.editbtn);
        await this.util.verifyElementPresent(this.abandonedheader);
        await this.util.verifyElementPresent(this.abandonedmsg);
    }

    /**
     * This method is used to click on Cancel button
     */
      async resetAbandonedClickNo(){
        await this.util.clickElement(this.abandonednobtn);

    }
    /**
     * This method is used to click on OK button
     */
    async resetAbandonedClickYes(){
        await this.util.clickElement(this.abandonedyesbtn);
    ...
    async updateCorrectPIN() {
        ...
        await this.util.verifyElementPresent(this.enterpinlbl)
        await this.util.verifyElementPresent(this.reenterpinchangelbl);
