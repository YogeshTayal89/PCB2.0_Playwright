...
export default class PCBPlayerRelationDetailsPage {
    ...
    sameinkedtxt:Locator;

    //MaxLinked
    maxlinkedheading:Locator;
    maxinkedtxt:Locator

    //NOLinked
    nolinkedheading:Locator;
    noinkedtxt:Locator
    ...
    constructor(page: Page) {
        ...
        this.unlinkbtn=this.framelocator.locator('td').filter({ hasText: 'link_off' }).first();
         ...
         //Same Player linked
         this.sameinkedtxt = this.framelocator.getByText('selected are the same.')

         //Max Linked
         this.maxlinkedheading=this.framelocator.getByRole('heading', { name: 'Information' });
         this.maxinkedtxt=this.framelocator.getByText('reached maximum number of allowable links');

         //NO Linked
         this.nolinkedheading=this.framelocator.getByRole('heading', { name: 'Information' });
         this.noinkedtxt=this.framelocator.getByText('not allowed.');
    ...
    async verifyMessgaeForAlreadyAddedPlayer(){
      ...
      /**
     * This method is used to verify the error message , if user try to link himself
     */
      async verifyMessgaeForLinkSamePlayer(){
        await this.util.verifyElementPresent(this.alreadylinkedheading)
        await this.util.verifyElementPresent(this.sameinkedtxt)
    }

    /**
     * This method is used to verify the error message , if Max link allowed limit reached
     */
    async verifyMessageMaxLinkedPlayerallowed(){
        await this.util.verifyElementPresent(this.maxlinkedheading)
        await this.util.verifyElementPresent(this.maxinkedtxt)
    }

     /**
     * This method is used to verify the error message , if NO link allowed limit reached
     */
     async verifyMessageNoLinkedPlayerallowed(){
        await this.util.verifyElementPresent(this.nolinkedheading)
        await this.util.verifyElementPresent(this.noinkedtxt)
    }
}
