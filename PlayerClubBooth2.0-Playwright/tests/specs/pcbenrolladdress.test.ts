...
test.describe('PCB Enroll Address Suite', () => {
    ...
    test.beforeEach(async ({ page },testInfo) => {
        ...
        if (testInfo.tags[3]=='@Auth'){
            configvalue='2';
            configreason='REQUIRED - Authorization pop for postal code'
        }
        else {
            configvalue='1';
            configreason='NONE - No Authorization for postal code'
        }

        await test.step(`Updated Configglobal config value to ${configvalue} of Item id 309 for ${configreason}`, async () => {
            const prerequisites=new PreRequisites();
            await prerequisites.updatepostalAuthorizeConfig(dburl,configvalue)
    ...
    test('Scenario Outline: As a user , I can enter all values in enroll address page @Address @Regression', async ({ page }) => {//Need to test
        const pcnenrolladdress = new PCBEnrollAddressPage(page);
   ...
   test('Scenario Outline: As a user , I can see the error message if we select already added type @refuseIDFlow @Address @Regression', async ({ page }) => {
    ...
    test('Scenario Outline: As a user , I can cancel adding new zip code process type @refuseIDFlow @Address @Regression', async ({ page }) => {
        const pcnenrolladdress = new PCBEnrollAddressPage(page);

        await test.step('Then I should see PCB enrollment address Page', async () => {
            await pcnenrolladdress.verifyEnrollAddressPageIsPresent();
        });
        await test.step('Then I can click on cancel button on Add new zip code pop up', async () => {
            await pcnenrolladdress.NewZipCodeAddCancel(enrollmentdata);
        });
        await test.step('Then i should see Error message on zip code',async()=>{
            await pcnenrolladdress.verifyErrorMessageOfValidZipcode();

        })
    });

    test.skip('Scenario Outline: As a user , I can add random new zip code process type @refuseIDFlow @Address @Regression', async ({ page }) => {
        const pcnenrolladdress = new PCBEnrollAddressPage(page);
        ...
        await test.step('Then I can click on cancel button on Add new zip code pop up', async () => {
            await pcnenrolladdress.AddNewZipCodeWithState(enrollmentdata);
        });
    ...
    test('Scenario Outline: As a user , I can add random new zip code for country not have State and AUTHORIZE is required @refuseIDFlow  @Address @Regression @Auth', async ({ page }) => {
        const pcnenrolladdress = new PCBEnrollAddressPage(page);
        ...
        await test.step('Then I can click on cancel button on Add new zip code pop up', async () => {
            await pcnenrolladdress.AddNewZipCodeWithoutState(enrollmentdata);
        });

        await pcnenrolladdress.verifyAuthorizeWithYesButton(username,password);
        await pcnenrolladdress.clickOnNextButton();
    });


    test('Scenario Outline: As a user , I can add random new zip code for country not have State and AUTHORIZATION is not required @refuseIDFlow @Address @Regression', async ({ page }) => {
        const pcnenrolladdress = new PCBEnrollAddressPage(page);
