...
test.describe('PCB Enroll Preview Suite', () => {
    ...
    test('Scenario Outline: As a user , I can navigate to pcb preview page @Preview @Smoke @Regression', async ({ page }) => {
        await test.step('Then I should see PCB enrollment priview Page', async () => {
    ...
    test('Scenario Outline: As a user , I can navigate to pcb preview page and Validate the entered value on Marketing details page @Preview @Smoke @Regression', async ({ page }) => {
        await test.step('Then I should see PCB enrollment priview Page', async () => {
            const pcbenrollpreviewpage = new PCBEnrollPreviewPage(page)
            await pcbenrollpreviewpage.verifyEnrollPreviewPageIsPresent();
        });

        await test.step('Then I should see the Marketing details filled and validate with entered details',async()=>{
            const pcbenrollpreviewpage = new PCBEnrollPreviewPage(page)
            await pcbenrollpreviewpage.verifyMarketingInfoIsPresent(enrollmentdata)
    ...
    test('Scenario Outline: As a user , I can save or Next button in preview page @Preview @Regression', async ({ page }) => {
        const pcbenrollpreviewpage = new PCBEnrollPreviewPage(page)
        await test.step('Then I should see PCB enrollment preview Page', async () => {
            await pcbenrollpreviewpage.verifyEnrollPreviewPageIsPresent();
        });

        await test.step('Then I can see save or next button', async () => {
            await pcbenrollpreviewpage.verifySaveNextBtn();
        });
