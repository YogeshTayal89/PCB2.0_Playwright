...
test.describe('PCB Player Detail Comments Tab Suite', () => {
    test.beforeAll(async () => {
        ...
        commentsdata = testdata.comments.testdata[0];
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await test.step('When I can do the search using any matching text', async () => {
            const pcbsearchpage = new PCBSearchPage(page);
            await pcbsearchpage.doSearch(commentsdata.playerid+ ","+commentsdata.fname);
        });
        await test.step('Then I can see the search result', async () => {
            const pcbsearchpage = new PCBSearchPage(page);
            await pcbsearchpage.verifySearchResultIsPresent(commentsdata);
        });
        await test.step('Then I can clear the comments', async () => {
            const prerequisite = new PreRequisites();
            await prerequisite.clearComments(dburl,commentsdata.playerid);
        });
    ...
    test('Scenario Outline: As a user , I can verify the elements in Add comment page @CommentsTab @Regression', async ({ page }) => {
    ...
    test('Scenario Outline: As a user , I should not see expired comments @CommentsTab @Regression', async ({ page }) => {
        ...
        await test.step('When I run query to make the expirydate is today date', async () => {
            ...
            await dbutils.doUpdateOrDelete("update playercomment set Expiration=(SELECT CONVERT(DATETIME, CONVERT(DATE, GETDATE()), 121)) where playerid ='" + commentsdata.playerid + "' and userid='30019' and Status='A'")
        });
